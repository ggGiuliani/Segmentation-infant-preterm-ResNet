...
def getData(path, foldlist):
    ...
    for i, image_path in enumerate(foldlist):
        ...
        mask = imread(file_name + '_mask' + '.png', as_grey=True)
        masks.append(mask)

       #image = imread(os.path.join(path, file_name + '_8' + '.png'), as_grey=True)
        image = imread(file_name + '_8' + '.png', as_grey=True)
        images.append(image)

        #image16 = imread(os.path.join(path, file_name + '.png'), as_grey=True)
        image16 = imread(file_name + '.png', as_grey=True)
...
def create_train_test_data():
    ...
    print("path passato: "+os.path.join(raw_data_path, images_dir))
